...
{
	Color color; 
	char c;									// c est un caractère de remplissage pour afficher une case
};
...
/*
	le type énuméré "Color" est spécifié dans le fichier cvm_25.h
...
{	
	...
	// MÉTHODE 1) -- caractère par caractère
	{
		...
		setcolor(map[box].color);					// la fonction setcolor est spécifiée dans cvm_25.h
	...
	// MÉTHODE 3) -- préfabriquer une string ligne 
	{
		...
		string ligne(CASE_X, map[box].c);			// préfabriquer une string
													// Nombre de répétitions et caractère à répéter.
